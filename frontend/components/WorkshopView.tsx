...
import { OSStatus } from '../types_erp';
    ...
    return (
        <div className="space-y-6">
            <header className="flex justify-between items-center">
                <div>
                    <h2 className="text-2xl font-bold text-slate-800 dark:text-white">{t('workshop.title')}</h2>
                    <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">{t('workshop.subtitle')}</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                {[
                    ...
                    <div key={i} className="bg-white dark:glass-panel border border-slate-200 dark:border-white/5 shadow-sm p-4 rounded-xl flex items-center justify-between">
                        <div>
                            <p className="text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider">{m.label}</p>
                            <p className="text-2xl font-bold text-slate-800 dark:text-white mt-1">{m.val}</p>
                        </div>
            ...
            <div className="bg-white dark:glass-panel border border-slate-200 dark:border-white/5 rounded-xl overflow-hidden shadow-sm">
                <div className="p-4 border-b border-slate-200 dark:border-white/5 flex items-center justify-between">
                    <h3 className="font-semibold text-slate-800 dark:text-white">{t('workshop.table.title')}</h3>
                    <div className="flex gap-2 text-sm text-slate-500 dark:text-slate-400">
                        <button className="px-3 py-1 hover:text-slate-800 dark:hover:text-white transition-colors">{t('workshop.table.all')}</button>
                        <button className="px-3 py-1 text-slate-800 dark:text-white bg-slate-100 dark:bg-white/10 rounded-md">{t('workshop.table.myOrders')}</button>
                    </div>
                </div>
                ...
                <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                        {MOCK_ORDERS.map((os) => {
                            ...
                            return (
                                <tr key={os.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors group cursor-pointer">
                                    <td className="px-6 py-4 font-medium text-slate-900 dark:text-white">{os.number}</td>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 rounded bg-slate-200 dark:bg-slate-700 bg-cover bg-center" style={{ backgroundImage: `url(${boat?.imageUrl})` }}></div>
                                            <div>
                                                <div className="text-slate-900 dark:text-white font-medium">{boat?.name}</div>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <button className="p-2 hover:bg-slate-100 dark:hover:bg-white/10 rounded-full text-slate-400 hover:text-slate-600 dark:hover:text-white transition-colors">
